let buttonsOn=true;
// Event Listeners for buttons as DOMContent is loaded in.
document.addEventListener("DOMContentLoaded", function(){
    let buttons = document.getElementsByTagName('button');
    for (let button of buttons) {
        button.addEventListener('click', function(){
            //Button for pressing the correct answer, checks if button is on to
...
// Function for each rounds start state, removes the 
// start screen and reveals the answer options and flag.
function gameDraw (){
    ...
    eraseButtonId();
    //Checks if roundlimit is reached, resets the board and  
    // randomly assigns correct id to one of the drawn countries.
    if(scoresObj.tot < roundLimit){
        let drawnCountries = countryCheck();
        let buttons = document.getElementsByClassName("answer");
        buttons.innerText= '';
        // Finds the country with the value of correct and adds the flag to the flag area.
...
// Function of picking the 4 random countries, and each time checks
// if the country is already drawn among the 4.
...
//Function that fires when the correct button is pressed
// Adds 1 to right + tot and updates average + 1sec delay
// to allow the player visual feedback on right/wrong answer.
...
//Function that fires when the wrong button is pressed
// Adds 1 to wrong + tot and updates average + 1sec delay
// to allow the player visual feedback on right/wrong answer.
...
// Function that fires when the game has reached the maximum amount of rounds
// Resets the country list + removes the game board and shows the start screen.
function gameOver(){
...
// Function that prints scores if the history is not yet full
//gives feedback for it being full.
function printScore(){
    if(attempt <= 20){
