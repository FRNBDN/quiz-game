...
let  countriesPicked = [];
for (let i=0; i<4; i++){
let pick = Math.round(Math.random()*countryList.length);
...
// Function that checks if there are any duplicates or previously
// used answers
function countryCheck (){
    let countriesPicked = countryPicker();
    console.log(countriesPicked);
    let countrySet = new Set();
    // making a Set of the countries to find duplicates
    for (let i = 0 ; i < countriesPicked.length ; i++){
        countrySet.add(countriesPicked[i].country);
    }
        console.log(countrySet);
    // checking set to see if it has the size of four
    // if not then there are duplicates.
  if(countrySet.size == 4){
    //removes the picked countries from the pool
   countriesPicked.sort((first, second)=> (first.id - second.id));
   console.log(countriesPicked);
   for (let i=3; i>=0; i--){
    console.log(countriesPicked[i].id);
    countryList.splice((countriesPicked[i].id), 1);
   }
